...
def hill_climbing(adj_matrix, make_file):
    ...
    # Get final cost
    final_cost = value(adj_matrix, current_path)

    # If make_file is True, write results to CSV
    if make_file:
        with open('hill_climbing_tsp.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([f"Total cost: {final_cost}, Nodes expanded: {nodes_expanded}, CPU Run Time: {cpu_run_time:.6f} seconds, Real-World Run Time: {real_run_time:.6f} seconds"])

    return current_path, final_cost, nodes_expanded, cpu_run_time, real_run_time
